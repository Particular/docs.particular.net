...
The code generator will create code for handlers and sending any messages that are in your design *the first time you build the solution*.  In the sample above, the code for handling the `SubmitOrder` message and publishing the `OrderAccepted` event was created because both of them were present in the ServiceMatrix design at initial build. 
...
The code below is generated for example saga class above. This is dynamic code that will be regenerated so it is not safe to edit it.
```C#
...
The message handling code of the Saga is implemented very much like the [component code above](#nservicebus-component-code).  The code will implement a handler in the saga for the messages handled in the ServiceMatrix design.  The Saga message handlers are generated using the same approach as in the earlier example..  
