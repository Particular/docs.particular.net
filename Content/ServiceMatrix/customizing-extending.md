...
To view the code for any of the components, use the drop-down within that component and select `View Code`.

#Customizing Message Definitions
NServiceBus messages are plain old CLR classes in C#.  Creating new messages is very easy in ServiceMatrix.  As you send commands or publish events you are prompted only for a name and ServiceMatrix does the rest.  When you build your solution, the messages are generated.  However, if you look at the generated classes you will notice they don't contain any properties.   

To modify a message, use the drop-down menu on the message, view the code and add whatever properties you wish.
...
The code below is based on a saga used to manage request response with another endpoint as shown.  This saga handles the `OrderAccepted` event then publishes the `SubmitPayment` request to a service.  The saga handles and correlates the `SubmitPaymentResponse`.
...
The message handling code is implemented very much like the component code above.  The code will implement a handler in the saga for the events and messages in the ServiceMatrix design.  It will include a partial method call into the 
