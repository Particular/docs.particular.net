...
The following have been into `NServiceBus.Core.dll` (part of the `NServiceBus` nuget)
...
So if these marker are still required they can be referenced via `NServiceBus.Core.dll`. 
...
NLog and Log4net integration have been extracted to external nuget packages. For more information see http://docs.particular.net/nservicebus/logging-in-nservicebus
...
#### 1. Install the `NServiceBus.Distributor.MSMQ` 
