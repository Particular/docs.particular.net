...
NServiceBus gives us the ability to not only send messages, but also [delay messages](/nservicebus/messaging/delayed-delivery). Delayed delivery gives us the ability to *send messages into the future* which is used by [Saga Timeouts](/nservicebus/sagas/timeouts).
