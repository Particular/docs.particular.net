 ...
 * `EntityMaximumSize(int)`: The maximum entity size in GB. The value must correspond to a valid value for the namespace type. See [the Microsoft documentation on quota's and limits](https://docs.microsoft.com/en-us/azure/service-bus-messaging/service-bus-quotas) for valid values.
 ...
 * `RuleNameShortener(Func<string, string>)`: By default rule names are derived from the message type's full name, which may exceed the maximum length of rule names. This callback allows to provide a replacement name for the rule. Rule names must adhere to the limits outlined in [TODO: find documentation if it exists]().
