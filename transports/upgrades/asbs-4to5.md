...
When switching an event to the new topic-per-event-type approach, the publisher and all subscribers must be upgraded together. Using the provided tool or infrastructure-as-code tools like Bicep, Terraform, or Pulumi, it is possible to set up the topic for a specific event, including all forwarding subscriptions, before rolling out the publisher update.
