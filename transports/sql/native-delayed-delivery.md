...
NOTE: The native delayed delivery of SQL Server transport, similar to the timeout manager, is available only in endpoints that are not configured as [send-only](/nservicebus/hosting/#self-hosting-send-only-hosting).
