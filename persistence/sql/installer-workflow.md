...
[NServiceBus Installers](/nservicebus/operations/installers.md) offer an automatic deployment model that requires elevated database permissions to execute correctly. In non-development environments, least-privilege access or stricter operational deployment control is often required, making automatic SQL installer script execution disallowed. To streamline development, installer execution can be enabled only on developers' machines. You can enable this behavior with the following code:

snippet: InstallerWorkflow

In this example, the `IsDevelopment` environment variable is used to enable script execution on development machines. You can also identify a development machine by checking a machine-name convention, using a command-line argument, or using another environment-specific toggle mechanism.
...
Depending on the organization and choice of tooling, the process can be fully automated, partially automated, or fully manual.
...
A typical workflow on a developer's machine consists of the following steps:
...
The workflow in a higher environment differs based on the specifics of an organization's process. For example, it is possible to allow endpoints [to automatically execute scripts](/persistence/sql/install.md#script-execution-in-non-development-environments) or [to take full control and execute them using custom code](/persistence/sql/install.md#script-execution-in-non-development-environments).

If you take full control of script execution, integrate script execution into the deployment pipeline the organization has in place, for example:

- When using Octopus Deploy, the scripts can be executed in a ["Step"](https://octopus.com/docs/deploying-applications/adding-steps).
- When using TeamCity, the scripts can be executed in a ["Build Step"](https://confluence.jetbrains.com/display/TCD8/Configuring+Build+Steps).
...
A sample workflow in that kind of environment can consist of the following steps:
 ...
 1. Scripts are executed in a higher environment as part of the existing deployment pipeline. Required settings, such as schemas or table prefixes, have different values in each environment. They are passed to scripts together with other configuration settings, such as a connection string.
