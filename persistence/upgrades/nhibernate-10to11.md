...
## Saga mapping loading

Saga mapping discovery has been tightened. Previously, all assemblies discovered during assembly scanning were registered with NHibernate, allowing saga mappings to be picked up from any scanned assembly, even if it did not contain saga entities. Now, saga entity types are resolved first from the registered saga metadata, and only assemblies that actually contain those saga entities are treated as the authoritative source of mappings.
