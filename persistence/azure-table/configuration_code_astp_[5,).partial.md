 ...
 * `EnableSecondaryKeyLookupForSagasCorrelatedByProperties`: To enable the compatibility mode and force the persister to find sagas by the secondary index property. If no more sagas are using the secondary index property `NServiceBus_2ndIndexKey`, the lookup by secondary key can be disabled.
