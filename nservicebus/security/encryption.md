...
NOTE: In NServiceBus Version 5 and below encryption is enabled by default and disables itself when there are no messages that require encryption. In NServiceBus Version 6 encryption is only enabled when explicitly configured with either `configuration.RijndaelEncryptionService` or `configuration.RegisterEncryptionService`.
