...
2. From a command line, execute `az login`, which will open a browser to complete the authentication to Azure. The Azure login must have [access to view metrics data](/servicepulse/usage-config.md#connection-setup-azure-service-bus-minimum-permissions) for the Azure Service Bus namespace.
