| <nobr>`--queueNameMasks`</nobr> | Masks sensitive information in the generated report. See [masking private data](/nservicebus/throughput-tool/#masking-private-data). |
...
| <nobr>`--unattended`</nobr> | Will not prompt for confirmation, so that the tool can be used from environments such as a continuous integration system. Specify `--customerName` as well, otherwise the tool still prompts for the customer name. |
