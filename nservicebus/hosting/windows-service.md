...
The endpoint can fail when using the [NServiceBus Host](nservicebus-host/) or [self hosting and implementing a critical error handler that exits the process](critical-errors/#default-action-handling-in-nservicebus) when a critical error occurs. Not having Windows Service Recovery configured will result in message processing to halt.
