...
`ServiceBusTriggeredEndpointConfiguration` loads some configuration values from the Azure Function host environment in the following order:
...
| Key                      | Value      | Notes     |
|--------------------------|------------|-----------|
| `AzureWebJobsServiceBus` | Connection string for the Azure ServiceBus namespace to connect to. | This value is required for `ServiceBusTriggerAttribute`. An alternative key can be passed in to the constructor. |
