...
> [!NOTE]
> The `bodyWriter` is automatically reset between handler attempts. Data written to it by a failing or non-matching handler will not affect subsequent handlers.
...
For a production-ready implementation that handles the [CloudEvents](https://cloudevents.io/) specification, see the [NServiceBus.Envelope.CloudEvents](/nservicebus/cloudevents.md) package.
