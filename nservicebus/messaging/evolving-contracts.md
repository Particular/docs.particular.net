...
* Update receivers that handle the old contract version by:
  * Processing the message assigning a default value to the missing properties.
  * When the new properties are required to correctly process the message, the implementation of the handler can be changed to retrieve the missing information, either from a storage or by requesting the data to another endpoint. Once the data becomes available, the message can be processed by sending a new message containing all information.
