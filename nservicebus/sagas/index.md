...
## Querying Saga Data
Sagas manage state of potentially long-running business processes. When we are building User interfaces that represent those business processes we suddenly feel the urge to query the saga data directly. It can be done, but we recommend to avoid it. It has the following implications:
...
* Querying the data might require additional indexes, resources etc. which need to be managed by the component issuing the query. It can influence saga performance and lead in high contention scenarios to more concurrency conflicts.
...
The recommended approach is to set up handlers that react to saga state changes and update one or more read model(s) for querying purposes. It reduces coupling to the internals of the specific saga persistence, removes contention and doesn't bypass the safeguard of the existing saga logic.