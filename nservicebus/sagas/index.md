...
The recommended approach is to set up handlers that react to saga state changes, published by the owning saga, and update one or more read model(s) for querying purposes. It reduces coupling to the internals of the specific saga persistence, removes contention and doesn't bypass the safeguard of the existing saga logic.