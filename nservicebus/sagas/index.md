...
Instead, if a saga needs data as a part of its message processing logic, the best way to address that would be to have the saga be started by a much earlier message and then collect all the data it needs by handling all the messages over time that provide that data. This may result in sagas that "live forever", ie don't have a message that would cause them to `MarkAsComplete()`. 

Since sagas are effectively just a record in a database when they're not processing messages, leaving a saga running "forever" is equivalent to not deleting a record from a database. This is no different from how regular business entities in a system aren't deleted.
