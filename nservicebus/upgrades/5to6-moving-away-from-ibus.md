...
* Unlike Microsoft API, where there is a mix of synchronous methods and asynchronous methods, where there is a particular need to differentiate the two, in NServiceBus all of the operations are fully async, and there are no synchronous counterparts. 

* Every operation that participates in the sending a message or receiving a message interacts with IO-bound resources like persisters returns a Task. Adding the `async` postfix to all these operations becomes redundant.
 
* [Changing the operations to be async](/nservicebus/upgrades/5to6-migrate-existing-handlers.md) in itself is a major breaking change which touches every API in NServiceBus internally. Changing all the method names to add the async suffix adds more work when trying to migrate to Version 6, for example, all the renames that would result in changing `Send` to `SendAsync`, `SendLocal` to `SendLocalAsync`, etc. 

* NServiceBus is not alone in taking this approach. Several other popular OSS libraries like Octokit are also doing this.
