---
title: Upgrade from version 4 to version 5
...
In the interest of "smaller changes are easier to verify" it is recommended that you do the update to .NET 4.5, and full migration to production, before updating to NServiceBus version 5.
...
<!-- import 4to5CustomConfigOverrides -->
...
<!-- import 4to5InterfaceMessageCreation -->
...
<!-- import 4to5ReflectionInterfaceMessageCreation -->
...
<!-- import 4to5ConfigureHowToFindSagaForUpgrade -->
...
<!-- import 4to5ConfigurePersistence -->
...
<!-- import 4to5BusExtensionMethodForHandler -->
...
<!-- import 4to5DefineCriticalErrorAction -->
...
<!-- import 4to5EncryptionServiceSimple -->
...
<!-- import 4to5EncryptionFromIEncryptionService -->
...
<!-- import 4to5StaticConfigureEndpoint -->


## Moved Performance monitoring and SLA to ConfigurationBuilder 

<!-- import 4to5PerformanceMonitoring -->
...
In version 4 it was also possible to use `Configure.Instance` to get access to an instance of `IBuilder`. If you were using `IBuilder` to access other run time instances, that also exist in the container, you should instead resolve them via [dependency injection](/nservicebus/containers/). Another alternative to using `IBuilder` is to configure NServiceBus to use a custom container instance. After starting the bus this instance can be used to resolve instances. Each of the container integrations have an [associated sample](/samples/containers/) that show how to pass in a custom instance of that container.


### To statically resolve IBus

One common use of `Configure.Instance` in version 4 was to access an instance of `IBus` in the static context, i.e. with this code `Configure.Instance.Builder.Build<IBus>()`. It is recommended to instead avoid using the static context and instead construct your instances via the container. For example to inject `IBus` add a constructor parameter of the type `IBus` and store that in a field for when it is needed.

If the class cannot be constructed though dependency injection you can store the instance of `IBus` returned from `Bus.Create(busConfiguration).Start()` in a static variable and access it in the global context.
  

### Static resolution via IWantToRunWhenBusStartsAndStops

If using the NServiceBus host or plugging into NServiceBus in a generic library the you will not have access to startup time via `Bus.Create(busConfiguration).Start()`. You can work around this by capturing both `IBuilder` and `IBus` via a `IWantToRunWhenBusStartsAndStops` and storing them statically.
...
Note that this approach should only be considered when resolving your components via the container is not an option.
