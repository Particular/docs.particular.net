...
If using the NServiceBus host or plugging into NServiceBus in a generic library the you will not have access to startup time via `Bus.Create(busConfiguration).Start()`. You can work around this by capturing both `IBuilder` and `IBus` via a `IWantToRunWhenBusStartsAndStops` and storing the statically.
