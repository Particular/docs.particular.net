...
NOTE: When trasitioning from `AsA_Client` to self-hosting the equivalent setting for the transport transaction mode is `None`. Make sure that message loss is acceptable, see [transport transactions](/transports/transactions) documentation for more details.
