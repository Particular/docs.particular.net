...
Switching to self-hosting can be done by using the [generic host](/nservicebus/hosting/extensions-hosting.md), Create a project that supports [Windows Service hosting](/nservicebus/hosting/windows-service.md) or [Docker container hosting](/nservicebus/hosting/docker-host/) with `dotnet new` using the [ParticularTemplates package](/nservicebus/dotnet-templates/), then move the relevant code and configuration to the new project.
