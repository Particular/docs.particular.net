...
The WCF integration using `WcfService` has been moved from the host to the separate Nuget package [NServiceBus.Wcf](https://www.nuget.org/packages/NServiceBus.Wcf/). That package must be used in order to use the WCF integration functionality in Version 7. The package is now fully independent from the host and can also be used in self-hosting scenarios.
