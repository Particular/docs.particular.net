...
## Selecting [Transports](/transports/) to use
...
Version 4 requires newer version of RavenDB. See also [RavenDB Version Compatibility](/persistence/ravendb/version-compatibility.md).


### Timeouts

`configure.RunTimeoutManagerWithInMemoryPersistence()` has been obsoleted. Enable [InMemory storage](/persistence/in-memory.md) for timeouts using `configure.UseInMemoryTimeoutPersister()`.
...
The interface `INeedInitialization` has been moved from `NServiceBus.Config.INeedInitialization` to `NServiceBus.INeedInitialization`. If a a `ambiguous reference` error occurs fully qualify the usage of that interface. See also [Lifecycle initialization](/nservicebus/lifecycle/ineedinitialization.md).
...
The [configuration API's for transactions](/transports/transactions.md) have changed.
