...
## Uniquely addressable endpoint instances

For certain scale out scenarios and callbacks, endpoint instances need to be uniquely addressable. Using `busConfiguration.ScaleOut().UniqueQueuePerEndpointInstance("-MyCustomSuffix");` would change the input queue name to include the specified suffix. With Version 6, the original input queue remains and an additional queue with the configured suffix is created. Uniquely addressable endpoints can consume messages from both (shared and unique) queues. Replies will automatically be sent to the instance specific queue. Configure an endpoint use an uniquely addressable queue with:
...
### Enabling and disabling First Level Retries (now called Immediate Retries) and Second Level Retries (now called Delayed Retries)

`FirstLevelRetries` and `SecondLevelRetries` are no longer features. As a result `configuration.DisableFeature<FirstLevelRetries>()` and `configuration.DisableFeature<SecondLevelRetries>()` cannot be used to disable them. Equivalent behavior can be achieved through code API by setting FLR and SLR retries to `0`: 
