...
<!-- import SetDeliveryConstraintDiscardIfNotReceivedBefore -->

Read the `TimeToBeReceived`

<!-- import ReadDeliveryConstraintDiscardIfNotReceivedBefore -->
...
<!-- import SetDeliveryConstraintNonDurable -->

Read the `Recoverable`

<!-- import ReadDeliveryConstraintNonDurable -->
...
Custom retry policies operate now on `IncomingMessage` instead of `TransportMessage`. For more information see [Custom Retry Policy](/nservicebus/errors/automatic-retries#second-level-retries-custom-retry-policy)

### Raw sending

In V5 you could use `ISendMessages` to do raw sends. In V6 `IDispatchMessages` was introduced. See the following snippet how raw sending could look like

<!-- import DispatcherRawSending -->
