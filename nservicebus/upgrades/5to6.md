...
If you are using it just include [the source code](https://github.com/Particular/NServiceBus/blob/5.2.5/src/NServiceBus.Core/CircuitBreakers/RepeatedFailuresOverTimeCircuitBreaker.cs) in your project.
...
Synchronous request-response capability, also known as the [Callbacks](/nservicebus/messaging/handling-responses-on-the-client-side.md), has been moved to a separate [NuGet package: `NServiceBus.Callbacks`](https://www.nuget.org/packages/NServiceBus.Callbacks/
). This package has to be reference by the requesting endpoint. The responding endpoint does not need any additional packages.

snippet: 5to6-Callbacks

In Version 6 callback messages routed has changed. The callback routing is based on unique endpoint instance ID. In order to continue using callbacks, this ID needs to be specified in the requester endpoint configuration

snippet: 5to6-Callbacks-InstanceId

NOTE: This identifier needs to be stable and it should never be hardcoded. For example reading from the configuration file or from the environment (e.g. role ID in Azure).

## SQL Server Transport

### Transactions
Transaction settings are now handled in the gransport level configuration. For details refer to [Transaction configuraton API](/nservicebus/upgrades/5to6.md#transaction-configuration-API) section.
...
The API and config structure for multi-schema configuration [were changed](/nservicebus/sqlserver/multiple-databases.md#single-database-with-multiple schemas). The `Queue Schema` parameter is no longer supported neither by config file nor code configuration. 

The schema for the current endpoint can be specified at compile time, using `DefaultSchema` method:

snippet:sqlserver-singledb-multischema

or at runtime by defining a custom connection factory (similar to "pull" approach in Version 2.x):
...
When using configuration file to specify schemas for other endpoints, their schemas should be placed in the `MessageEndpointMappings` section: 
