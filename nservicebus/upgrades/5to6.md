...
It is recommended to update to .NET 4.5.2 and perform a full migration to production _before_ updating to NServiceBus Version 6.
...
The `IWantToRunWhenBusStartsAndStops` interface is obsolete. When self-hosting, call any startup code after `Endpoint.Create` or any cleanup code after `Endpoint.Stop`. When using either [NServiceBus.Host](/nservicebus/hosting/nservicebus-host) or [NServiceBus.Host.AzureCloudService](/nservicebus/azure/hosting-in-azure-cloud-services.md), use the host's `IWantToRunWhenEndpointStartsAndStops` interface.
...
When using either [NServiceBus.Host](/nservicebus/hosting/nservicebus-host) or [NServiceBus.Host.AzureCloudService](/nservicebus/azure/hosting-in-azure-cloud-services.md), implementations of `IWantToRunWhenEndpointStartsAndStops` are not started and stopped on a dedicated thread. They are executed on the thread starting and disposing the endpoint. It is the responsibility of the implementing class to execute its operations in parallel if needed (i.e. for CPU bound work). Failure to do so will prevent the endpoint from being started and/or disposed. More information about the startup and shutdown sequence can be found in the [Startup and Shutdown sample](/samples/startup-shutdown-sequence/).

All `IWantToRunWhenEndpointStartsAndStops` implementations are started before the transport and any satellites are started. Furthermore, they are stopped after the transport and the satellites have been stopped. This ensures that, during the startup and stop phase, inside an implementation of `IWantToRunWhenEndpointStartsAndStops` no messages will be received by the endpoint.
