...
While the [Dispose Pattern](https://msdn.microsoft.com/en-us/library/b1yfkh5e.aspx) can no longer be used (since `IEndpointInstance` does not implement [IDisposable](https://msdn.microsoft.com/en-us/library/system.idisposable.aspx)) this is actually not a common use case since since in most [hosing scenarios](/nservicebus/hosting/) the startup code is not in the same method as the shutdown code. For example 
...
This same functionality and more can be achieved using the [message processing pipeline](/nservicebus/pipeline/customizing-v6.md). See also [Customizing error handling with the pipeline](/nservicebus/pipeline/customizing-error-handling.md).
