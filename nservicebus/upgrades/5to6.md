...
Synchronous request-response capability, also known as the callbacks, has been removed from the `NServiceBus` package in V6. There is a separate `NServiceBus.Callbacks` NuGet package that needs to be installed in the requester endpoint in order to use the feature. The responding endpoint does not need any additional packages.

snippet: 5to6-Callbacks

In V6 we also changed the way callback messages are routed. In V6 callback routing is based on unique endpoint instance ID. In order to continue using callbacks, you need to assign this IDs on the requester side
