...
Default behaviour for assembly scanning has changed in version 6. NServiceBus is no longer scanning assemblies in nested folders. Changing default behaviour is covered by [Assembly Scanning API](/nservicebus/hosting/assembly-scanning.md) documentation.

Assembly scanning API was changed to discourage inclusion of assemblies (whitelisting) and only supports exclusion options (blacklisting). Creating blacklists provides a better control over assemblies not to scan than trying to get the whitelist correctly. Whitelisting has showed a high rate of errors when necessary assemblies were left out, resulting in unexpected behaviour. All whitelisting options are documented as [deprecated options](/nservicebus/hosting/assembly-scanning.md#options-deprecated-from-version-6-and-later) and are not available in version 6.
