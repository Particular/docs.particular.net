...
In versions 6 and above the `TimeoutManager` does not expose any error notifications. When an error occurs during processing of a deferred message by the `TimeoutManager`, the message will be retried and possibly moved to the error queue but user will not be notified about these events.
