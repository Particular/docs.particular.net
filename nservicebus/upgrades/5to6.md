...
In Version 6 the callback handling on [broker transports](/nservicebus/transports/#types-of-transports-broker-transports) is no longer based on machine host name. In order to continue using callbacks when using a broker transport a unique endpoint instance ID has to be provided. 
