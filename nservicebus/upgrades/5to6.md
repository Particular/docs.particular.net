...
We've separated the synchronous request-response capability, also known as callbacks, from the NServiceBus core. This means that you now have to explicitly install our new [NServiceBus.Callbacks](https://www.nuget.org/packages/NServiceBus.Callbacks/) package to get access to callback support. In contrast to previous versions this API allows you to easily access the response message and is asynchronous by default. The extension methods provided by the new callback package are only available on the message session, so it is no longer possible to use callbacks inside handlers or sagas. The differences in the API are fully covered in [handling responses on the client side](/nservicebus/messaging/handling-responses-on-the-client-side.md).

The callbacks package has to be referenced only by the requesting endpoint. The responding endpoint does not need any additional packages.
