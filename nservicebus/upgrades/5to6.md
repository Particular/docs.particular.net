...
The interface `IWantQueueCreated` has been replaced by the `QueueBindings` class. The instance of `QueueBindings` can be accessed via the settings API (`ReadOnlySettings` interface). For more information how to declare and create queues see [Declare and create queues](/nservicebus/extending/queuecreation.md).
