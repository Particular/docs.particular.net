...
In certain scenarios, e.g. when using callbacks, the endpoint instances need to be uniquely addressable. The unique instance discriminator can be specified using code API:

snippet: UniqueQueuePerEndpointInstanceDiscriminator

Refer to the [Message routing](/nservicebus/messaging/handling-responses-on-the-client-side?version=callbacks_1#message-routing) section for more information.
