 ...
 * Behavior instances are created once, when the endpoint starts up, and then cached for the lifetime of the endpoint. This is true even if the behavior is resolved from the container when its type is registered as `InstancePerCall` or `InstancePerUnitOfWork`.
