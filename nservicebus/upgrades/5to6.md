...
There is also a change in the parameters, giving you access to the `IMessageHandlerContext`, which wraps any methods that used to be called from `IBus`. Use the `IMessageHandlerContext` to send and publish messages.
