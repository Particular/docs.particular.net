...
Previous versions of NServiceBus forced usage of `Address` class as a representation of an address in all transports despite the fact that the form *queue@host* did not match most of the transports. In Version 6 the transport address is represented as an opaque `string` and NServiceBus core no longer makes any assumptions on its form.
