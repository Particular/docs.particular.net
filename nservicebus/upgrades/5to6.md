...
Saga persisters `ISagaPersister` and finders `IFindSagas` introduce a new method parameter `SagaPersistenceOptions`. The options give access to the saga metadata and float countext bound objects into the saga persisters and finders. Therefore enabling to acquire everything which is added to the context during the pipeline execution.
