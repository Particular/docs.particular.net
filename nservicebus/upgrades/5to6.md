...
Saga persisters `ISagaPersister` and finders `IFindSagas` introduce a new method parameter `SagaPersistenceOptions`. These options give access to the saga metadata and pipeline context within saga persisters and finders. This enables custom saga persisters and custom saga finders to have access to everything which is added to the context during message pipeline exectuion.
