...
Previously it was possible to access `IMessageContext` both over `UnicastBus` and the `IBus` interface. Accessing the current message context over `UnicastBus` is no longer supported. To get access to the MessageId, the ReplyToAddress or the headers use the message handler context.
