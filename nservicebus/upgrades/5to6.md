...
Synchronous request-response capability, also known as the callbacks, has been moved to a separate NuGet package called `NServiceBus.Callbacks`. This package has to be reference by the requesting endpoint. The responding endpoint does not need any additional packages.

snippet: 5to6-Callbacks

In version 6 we also changed the way callback messages are routed. The callback routing is based on unique endpoint instance ID. In order to continue using callbacks, this ID needs to be specified in the requester endpoint configuration
