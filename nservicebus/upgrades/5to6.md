...
While using a suppressed transaction scope to requests sends to be dispatched immediately is still supported we recommend that you switch to our new explicit API for immediate dispatch. You can read more on this topic [here](/nservicebus/messaging/immediate-dispatch.md).


## SubscriptionEventArgs has been deprecated
Version 5 introduced a undocumented way to get the list of subscribers when publishing a message on the transports using [persistence based pub/sub](/nservicebus/messaging/publish-subscribe/#mechanics-persistence-based). This is no longer available, please contact us should you need this type of information in Version 6.