...
Instead use `context.Settings.GetRequiredTransactionSupportForReceives() != TransportTransactionMode.TransactionScope` to determine if distributed transactions are suppressed.


#### IsTransactional

Instead use `context.Settings.GetRequiredTransactionSupportForReceives() != TransportTransactionMode.None` to determine if transactions are enabled.