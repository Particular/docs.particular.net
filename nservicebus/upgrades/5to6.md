...
In Version 6 the callback handling on [broker transports](/nservicebus/transports.md/#types-of-transports-broker-transports) is no longer based on machine host name. In order to continue using callbacks when using a broker transport a unique endpoint instance ID has to be provided. 

snippet: 5to6-Callbacks-InstanceId

NOTE: This ID should never be hardcoded, e.g. it can be read from the configuration file or from the environment (e.g. role ID in Azure) so that it can be changed by operations people without requiring a code change and redeployment.
