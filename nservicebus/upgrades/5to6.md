...
If the extensibility provided by `IWantToRunWhenBusStartsAndStops` is still required it can be achieved via other means such as shown in [Using MEF or Reflection to customize NServiceBus](/samples/plugin-based-config/)
...
#### Using NServiceBus.Host.AzureCloudService
