...
- Tasks can be run concurrently if a task its execution duration exceeds the schedule interval. If a task may not overlap then either cancel either the old or new task or wait for the current task to complete.
