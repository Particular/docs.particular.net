...
When a saga containing a property of an unsupported type is persisted, an exception containing a following information is thrown: `The property type 'Your_property_name' is not supported in windows azure table storage`. If an object of a complex type is required to be stored, then it's a user responsibility to serialize/deserialize the value to a `byte[]` property.
