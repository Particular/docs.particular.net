...
NServiceBus uses the MD5 hash algorithm to generate deterministic unique identifiers for endpoints, also known as HostIds. MD5 is not FIPS compliant. Using a FIPS-approved hashing algorithm like SHA256 to generate a HostId will allow an NServiceBus endpoint to run under a FIPS policy.
