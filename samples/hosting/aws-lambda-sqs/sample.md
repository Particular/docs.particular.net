...
There could be a scenario when it is needed to dispatch a message using an AWS Lambda but from outside of a message handler, like reacting to events other than messages in a queue. For example, responding to a S3 bucket change or to an HTTP call. This sample also demonstrates this use case.
