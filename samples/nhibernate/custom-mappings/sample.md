...
Sometimes, the database schema generated by `NServiceBus.NHibernate` is insufficient for saga data types. For example, storing strings with different lengths, using complex types, needing to store DateTime values with high precision, or tweaking the eager versus lazy loading rules might be better options. A custom mapping will be required to achieve these changes.
...
Other options exist, but these are the most frequently used. Minor performance differences can exist between these options, but they will occur during endpoint startup, not when the endpoint is processing messages.
