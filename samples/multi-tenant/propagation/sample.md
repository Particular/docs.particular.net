...
The second behavior attaches the tenant information header to outgoing messages based on the pipeline context. This behavior executes as part of the message send pipeline.
...
In addition, both the `OrderAccepted` and `OrderPlaced` message handlers log the tenant ID header value extracted from the incoming message.
