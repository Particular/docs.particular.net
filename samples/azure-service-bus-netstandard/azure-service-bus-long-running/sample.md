...
When processing takes a long time, [message lock renewal](/transports/azure-service-bus/configuration#lock-renewal) is possible. The message lock renewal operation is initiated by the Azure Service Bus client, not the broker. If it fails after all the retries (e.g. due to a connection loss), the lock won't be re-acquired, and the message will become unlocked and available for processing. Lock renewal should be treated as best-effort and not as a guaranteed operation.
