...
summary: Transactional Session sample that illustrates how to send messages and modify data with Entity Framework in an atomic manner using ASP.NET Core and NServiceBus send-only endpoints.
...
> [!Note]
> From 8.2.0, `NServiceBus.Persistence.Sql.TransactionalSession` can be used in outbox-enabled send-only endpoints. Refer to the [documentation](/nservicebus/transactional-session/#send-only) for more details.
