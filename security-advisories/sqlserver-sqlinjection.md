...
The exploitation of this vulnerability requires that all of the conditions below are met at the same time:
 ...
 1. The receiving endpoint sends a message based on `ReplyToAddres`s header - for example, by using the `Reply` method of `IBus` or using the `IMessageHandlerContext` parameter available in the message handlers when using NServiceBus Version 6 and above.
