...
When an audit message that has failed import is detected in the ServiceControl database, the [**Audit Message Ingestion** custom check](servicecontrol/servicecontrol-instances.md#self-monitoring-via-custom-checks-failed-imports) is marked as failed to bring the failed audit import(s) to the administrator's attention.
