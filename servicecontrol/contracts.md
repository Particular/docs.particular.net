...
ServiceControl uses [Event Publishing](/nservicebus/messaging/publish-subscribe/) to send alerts to all subscribers. If using a [persistence based transport](/nservicebus/messaging/publish-subscribe/#mechanics-persistence-based) then ServiceControl will keep an internal reference to each subscriber. If a subscriber for an alert cannot be contacted ServiceControl will log the following error "Failed dispatching external integration event". An event will also be published and displayed in ServicePulse dashboard that contains the following "'EVENTTYPE' failed to be published to other integration points. Reason for failure: REASON".
