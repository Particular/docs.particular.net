...
NOTE: It's important that your integration endpoints doesn't use the same `error` and `audit` queue as your "business" endpoints since this might risk failures in the integration endpoint to cause an infinite feedback loop. Audited messages can cause confusion when searching for messages using ServiceInsight. See also [adjust error queue settings](/nservicebus/errors/) and [audit queue settings](/nservicebus/operations/auditing.md).
