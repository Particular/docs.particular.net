...
sealed class ComponentDocsMover
{
    ...
    public MovePlan BuildPlan()
    {
        ...
        foreach (var candidate in candidates)
        {
            ...
            var updatedContent = movedContentRewriter.RewriteContent(withImages);
        ...
        IReadOnlyList<CrossDocLinkRewrite> crossDocRewrites)
    {
        ...
        var duplicateSegmentExamples = documents
            .Where(d => HasRepeatedSegment(d.NewRelativePath))
            .Select(d => d.NewRelativePath)
            .Take(3)
            .ToList();
        if (duplicateSegmentExamples.Count > 0)
        {
            items.Add(
                $"Destination paths contain repeated segments (likely --from is too broad). Examples: {string.Join("; ", duplicateSegmentExamples)}. Consider a narrower --from and re-run.");
        }
    ...
    static bool HasRepeatedSegment(string relativePath)
    {
    ...
    static string CreateIndexContent(string indexRelativePath)
    {
        ...
        return string.Join('\n',
            "---",
            $"title: \"TODO: {title}\"",
            "summary: \"TODO: replace placeholder scaffolded by component-docs-mover.\"",
            $"reviewed: {reviewed}",
            "---",
            "",
            $"> **TODO (scaffolded by `component-docs-mover`):** replace this placeholder index for `{directory}` with real content, or delete this file if the directory does not need its own index page.");
    }
